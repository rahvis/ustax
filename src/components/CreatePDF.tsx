...
export default function CreatePDF(): ReactElement {
  ...
  const federalReturn = async (e: FormEvent<Element>): Promise<void> => {
    ...
    const r2 = await r1.mapAsync((bytes) =>
      savePDF(bytes, federalFileName, formData)
    );
    return r2.orThrow();
  };
  ...
  const stateReturn = async (): Promise<void> => {
    ...
    const r2 = await r1.mapAsync((bytes) =>
      savePDF(bytes, stateFileName, formData)
    );
    return r2.orThrow();
  };
  ...
  return (
    <div>
      ...
      <form tabIndex={-1}>
        <Helmet>
          <title>Print Copy to File | Results | ITIN Help</title>
        </Helmet>
        {printActions}
        {navButtons}
      </form>
